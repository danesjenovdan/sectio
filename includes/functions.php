...
function yourls_insert_link_in_db( $url, $keyword, $title = '', $start, $end, $context, $shares, $tags, $geo, $loop, $referer, $customlink ) {
	global $ydb;
	...
	$context   = addslashes( yourls_sanitize_title( $context ) );
	$shares   = addslashes( yourls_sanitize_title( $shares ) );
	$tags   = addslashes( yourls_sanitize_title( $tags ) );
	$loop   = addslashes( yourls_sanitize_title( $loop ) );
	$referer   = addslashes( yourls_sanitize_title( $referer ) );
	$customlink   = addslashes( yourls_sanitize_title( $customlink ) );

	$table = YOURLS_DB_TABLE_URL;
	$timestamp = date('Y-m-d H:i:s');
	$ip = yourls_get_IP();
	$insert = $ydb->query("INSERT INTO `$table` VALUES('$keyword', '$url', '$title', '$timestamp', '$ip', 0, '$start', 
		'$end', '$context', '$shares', '$tags', '$geo', '$loop', '$referer', '$customlink');");
	
	yourls_do_action( 'insert_link', (bool)$insert, $url, $keyword, $title, $timestamp, $ip, $start, 
		$end, $context, $shares, $tags, $geo, $loop, $referer, $customlink );
...
function yourls_add_new_link( $url, $keyword = '', $title = '', $start, $end, $context, $shares, $tags, $geo, $loop, $referer, $customlink ) {
	global $ydb;
...
*/	
		...
		if ( $keyword ) {
			...
			if ( !yourls_keyword_is_free($keyword) ) {
				...
				// all clear, store !
				yourls_insert_link_in_db( $url, $keyword, $title, $start, $end, $context, $shares, $tags, $geo, $loop, $referer, $customlink );
				...
				if ($customlink != '') {
					$return['shorturl'] = YOURLS_SITE .'/'. $customlink .'/'. $keyword;
				}else{
		...
		// Create random keyword	
			...
			do {
				...
				$add_url = @yourls_insert_link_in_db( $url, $keyword, $title, $start, $end, $context, $shares, $tags, $geo, $loop, $referer, $customlink );
				$ok = ($free && $add_url);
...
function yourls_get_vidmo_url( $keyword, $notfound = false ) {
...
//	$uurl = $infos['url'].'&start='.$infos['start'].'&end='.$infos['end'];
	//&t=4s&vidmo:start4&vidmo:end12
	$uurl = $infos['url'].'&t='.$infos['start'].'s&vidmo:start='.$infos['start'].'&vidmo:end='.$infos['end'];
